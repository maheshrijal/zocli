...
func runAuthLogin(args []string) error {
	...
	browser := fs.String("browser", "chrome", "Browser profile to use (chrome, chromium, brave, edge, helium)")
...
func runAuthImport(args []string) error {
	...
	browser := fs.String("browser", "chrome", "Browser profile to read (chrome, chromium, brave, edge, helium)")
	browserPath := fs.String("browser-path", "", "Path to browser executable (optional)")
